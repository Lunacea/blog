...
Use `Field` with `orientation="horizontal"` for settings pages. Use `Field.FieldLabel` with
`class="sr-only"` for visually hidden labels.
...
Use `Field.FieldSet` + `Field.FieldLegend` for related checkboxes, radios, or switches — not `div`
with a heading:
