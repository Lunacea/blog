...
Deno.test("memory repository toggles once per actor and aggregates counts", async () => {
  ...
  await repository.set("article:test", "actor-a", true);
  await repository.set("article:test", "actor-a", true);
  await repository.set("article:test", "actor-b", true);
  assertEquals((await repository.get("article:test", "actor-a")).count, 2);
  assertEquals((await repository.get("article:test", "actor-a")).selected, true);

  await repository.set("article:test", "actor-a", false);
  assertEquals((await repository.get("article:test", "actor-a")).count, 1);
});
