...
<header class="relative z-(--z-header) border-b border-rule" data-ready={ready}>
  <div class="mx-auto flex w-full max-w-content flex-wrap items-center justify-between gap-x-(--space-6) gap-y-(--space-1) px-(--layout-gutter)">
    <a class="flex shrink-0 items-baseline font-stretch-104% text-small leading-none font-strong tracking-heading text-ink uppercase no-underline hover:no-underline" href="/">
      <span class="sr-only">Lunacea</span>
      <span aria-hidden="true">LUNA</span>
      <span class="relative inline-block" aria-hidden="true"><span class="invisible">C</span>
